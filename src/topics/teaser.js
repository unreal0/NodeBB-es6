...
module.exports = (Topics) => {
	...
	Topics.getTeasers = (topics, uid, callback) => {
		...
		async.waterfall([
			...
			(_postData, next) => {
				postData = _postData;
				var uids = _.uniq(postData.map(post => post.uid));
	...
	Topics.getTeasersByTids = (tids, uid, callback) => {
		if (typeof uid === 'function') {
	...
	Topics.getTeaser = (tid, uid, callback) => {
		if (typeof uid === 'function') {
			winston.warn('[Topics.getTeaser] this usage is deprecated please provide uid');
			callback = uid;
			uid = 0;
		}
		Topics.getTeasersByTids([tid], uid, (err, teasers) => {
