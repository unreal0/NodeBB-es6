...
function getFiles(pluginData, type, callback) {
	...
	var files = pluginData[type].map(file => (
...
function resolveModulePath(basePath, modulePath, callback) {
	...
	file.exists(currentPath, (err, exists) => {
		if (err) {
			return callback(err);
		}
		if (exists) {
...
function getScripts(pluginData, target, callback) {
	...
	var scripts = [];
	async.eachSeries(input, (filePath, next) => {
		resolveModulePath(pluginData.path, filePath, (err, modulePath) => {
			if (err) {
				return next(err);
			}
...
function getModules(pluginData, callback) {
	...
	var modules = {};
	async.each(Object.keys(pluginModules), (key, next) => {
		resolveModulePath(pluginData.path, pluginModules[key], (err, modulePath) => {
			if (err) {
				return next(err);
			}
...
function getSoundpack(pluginData, callback) {
	...
	async.each(Object.keys(spack.sounds), (name, next) => {
		var soundFile = spack.sounds[name];
		file.exists(path.join(soundpack.dir, soundFile), (err, exists) => {
			if (err) {
				return next(err);
			}
			if (!exists) {
...
function getLanguageData(pluginData, callback) {
	...
	file.walk(pathToFolder, (err, paths) => {
		if (err) {
			return callback(err);
		}
