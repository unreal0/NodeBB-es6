...
module.exports = (Plugins) => {
	...
	*/
	Plugins.registerHook = (id, data, callback) => {
		...
		if (data.hook && data.method) {
			...
			if (typeof data.method === 'string' && data.method.length > 0) {
				method = data.method.split('.').reduce((memo, prop) => {
					if (memo && memo[prop]) {
						return memo[prop];
					}
	...
	Plugins.fireHook = (hook, params, callback) => {
		...
		try {
			...
			winston.error('[plugins] error hookType: ' + hookType + ', hook : ' + hook);
			callback(err);
		}
	};
	...
	function fireFilterHook(hook, hookList, params, callback) {
		...
		async.reduce(hookList, params, (params, hookObj, next) => {
	...
	function fireActionHook(hook, hookList, params, callback) {
		...
		async.each(hookList, (hookObj, next) => {
			if (typeof hookObj.method !== 'function') {
				if (global.env === 'development') {
					winston.warn('[plugins] Expected method for hook \'' + hook + '\' in plugin \'' + hookObj.id + '\' not found, skipping.');
				}
	...
	function fireStaticHook(hook, hookList, params, callback) {
		...
		async.each(hookList, (hookObj, next) => {
			if (typeof hookObj.method === 'function') {
				var timedOut = false;
				winston.info('[hooks]  hook: ' + hook + ' id: ' + hookObj.id);
	...
	Plugins.hasListeners = hook => !!(Plugins.loadedHooks[hook] && Plugins.loadedHooks[hook].length > 0);
};
