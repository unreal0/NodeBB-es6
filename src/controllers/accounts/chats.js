...
chatsController.get = (req, res, callback) => {
	...
	async.waterfall([
		...
		(data) => {
			...
			room.users = data.users.filter(user => user && parseInt(user.uid, 10) && parseInt(user.uid, 10) !== req.uid);
...
chatsController.redirectToChat = (req, res, next) => {
