...
groupsController.list = (req, res, next) => {
	...
	groupsController.getGroupsFromSet(req.uid, sort, 0, 14, (err, data) => {
		if (err) {
			return next(err);
		}
...
groupsController.getGroupsFromSet = (uid, sort, start, stop, callback) => {
...
groupsController.details = (req, res, callback) => {
	var groupName;
	async.waterfall([
		(next) => {
...
groupsController.members = (req, res, callback) => {
	var groupName;
	async.waterfall([
		(next) => {
			groups.getGroupNameByGroupSlug(req.params.slug, next);
...
groupsController.uploadCover = (req, res, next) => {
