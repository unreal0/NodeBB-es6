...
topicsController.get = (req, res, callback) => {
	...
	async.waterfall([
		...
		(topicData) => {
			...
			if (req.uid) {
				topics.markAsRead([tid], req.uid, (err, markedRead) => {
					if (err) {
						return callback(err);
					}
					if (markedRead) {
...
function addTags(topicData, req, res) {
	var postAtIndex = topicData.posts.find(postData => parseInt(postData.index, 10) === parseInt(Math.max(0, req.params.post_index - 1), 10));
...
topicsController.teaser = (req, res, next) => {
	var tid = req.params.topic_id;
...
topicsController.pagination = (req, res, callback) => {
	var tid = req.params.topic_id;
	var currentPage = parseInt(req.query.page, 10) || 1;
