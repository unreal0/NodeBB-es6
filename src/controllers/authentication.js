...
authenticationController.registerComplete = (req, res, next) => {
...
authenticationController.registerAbort = (req, res) => {
...
function continueLogin(req, res, next) {
	passport.authenticate('local', (err, userData, info) => {
		...
		if (passwordExpiry && passwordExpiry < Date.now()) {
			...
			user.reset.generate(userData.uid, (err, code) => {
				if (err) {
					return helpers.noScriptErrors(req, res, err.message, 403);
				}
			...
			authenticationController.doLogin(req, userData.uid, (err) => {
				if (err) {
					return helpers.noScriptErrors(req, res, err.message, 403);
				}
...
authenticationController.doLogin = (req, uid, callback) => {
	if (!uid) {
		return callback();
	}
	async.waterfall([
...
authenticationController.localLogin = (req, username, password, next) => {
	if (!username) {
...
function getBanInfo(uid, callback) {
	...
	async.waterfall([
		...
		(_banInfo, next) => {
			...
			translator.translate('[[user:info.banned-no-reason]]', (translated) => {
