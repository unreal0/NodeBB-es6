...
Controllers.reset = (req, res, next) => {
	if (req.params.code) {
		async.waterfall([
			(next) => {
...
Controllers.login = (req, res, next) => {
	...
	if (req.uid) {
		user.getUserFields(req.uid, ['username', 'email'], (err, user) => {
			if (err) {
				return next(err);
			}
...
Controllers.registerInterstitial = (req, res, next) => {
	...
	async.waterfall([
		...
		(data, next) => {
			...
			var renders = data.interstitials.map(interstitial => async.apply(req.app.render.bind(req.app), interstitial.template, interstitial.data || {}));
...
Controllers.confirmEmail = (req, res) => {
	user.email.confirm(req.params.code, (err) => {
		res.render('confirm', {
...
Controllers.outgoing = (req, res, next) => {
...
Controllers.termsOfUse = (req, res, next) => {
