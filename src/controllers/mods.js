...
modsController.flags.list = (req, res, next) => {
	var filters;
	var hasFilter;
	...
	async.waterfall([
		...
		(results, next) => {
			...
			filters = validFilters.reduce((memo, cur) => {
			...
			if (res.locals.cids) {
				if (!filters.cid) {
					...
					filters.cid = filters.cid.filter(cid => res.locals.cids.indexOf(String(cid)) !== -1);
		...
		(data) => {
			...
			if (res.locals.cids) {
				data.categories = data.categories.filter(category => res.locals.cids.indexOf(String(category.cid)) !== -1);
			}

			// Minimal returned set for templates.js
			data.categories = data.categories.reduce((memo, cur) => {
				if (!res.locals.cids) {
...
modsController.flags.detail = (req, res, next) => {
	async.parallel({
		isAdminOrGlobalMod: async.apply(user.isAdminOrGlobalMod, req.uid),
		moderatedCids: async.apply(user.getModeratedCids, req.uid),
...
modsController.postQueue = (req, res, next) => {
	async.waterfall([
		(next) => {
