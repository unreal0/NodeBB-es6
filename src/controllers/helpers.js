...
helpers.noScriptErrors = (req, res, error, httpStatus) => {
	...
	middleware.buildHeader(req, res, () => {
...
helpers.buildFilters = (url, filter) => [{
...
helpers.notAllowed = (req, res, error) => {
...
helpers.buildCategoryBreadcrumbs = (cid, callback) => {
	var breadcrumbs = [];

	async.whilst(() => parseInt(cid, 10), (next) => {
		categories.getCategoryFields(cid, ['name', 'slug', 'parentCid', 'disabled', 'isSection'], (err, data) => {
			if (err) {
				return next(err);
			}
...
helpers.buildBreadcrumbs = (crumbs) => {
	var breadcrumbs = [
		{
			text: '[[global:home]]',
			url: nconf.get('relative_path') + '/',
...
helpers.getWatchedCategories = (uid, selectedCid, callback) => {
