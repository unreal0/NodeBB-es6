...
apiController.loadConfig = (req, callback) => {
	var config = {};
...
apiController.getConfig = (req, res, next) => {
	async.waterfall([
		(next) => {
...
apiController.getPostData = (pid, uid, callback) => {
	async.parallel({
		privileges: (next) => {
...
apiController.getTopicData = (tid, uid, callback) => {
	async.parallel({
		privileges: (next) => {
...
apiController.getCategoryData = (cid, uid, callback) => {
	async.parallel({
		privileges: (next) => {
...
apiController.getObject = (req, res, next) => {
	var methods = {
	...
	method(req.params.id, req.uid, (err, result) => {
		if (err || !result) {
			return next(err);
		}
...
apiController.getModerators = (req, res, next) => {
	categories.getModerators(req.params.cid, (err, moderators) => {
		if (err) {
			return next(err);
		}
