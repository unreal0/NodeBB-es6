...
uploadsController.upload = (req, res, filesIterator) => {
	...
	async.map(files, filesIterator, (err, images) => {
...
uploadsController.uploadPost = (req, res, next) => {
	uploadsController.upload(req, res, (uploadedFile, next) => {
...
uploadsController.uploadThumb = (req, res, next) => {
...
uploadsController.uploadGroupCover = (uid, uploadedFile, callback) => {
	if (plugins.hasListeners('filter:uploadImage')) {
		return plugins.fireHook('filter:uploadImage', {
			image: uploadedFile,
			uid: uid,
...
uploadsController.uploadFile = (uid, uploadedFile, callback) => {
	if (plugins.hasListeners('filter:uploadFile')) {
		return plugins.fireHook('filter:uploadFile', {
			file: uploadedFile,
			uid: uid,
