...
uploadsController.uploadCategoryPicture = (req, res, next) => {
...
uploadsController.uploadFavicon = (req, res, next) => {
	var uploadedFile = req.files.files[0];
	var allowedTypes = ['image/x-icon', 'image/vnd.microsoft.icon'];

	if (validateUpload(req, res, next, uploadedFile, allowedTypes)) {
		file.saveFileToLocal('favicon.ico', 'system', uploadedFile.path, (err, image) => {
			file.delete(uploadedFile.path);
			if (err) {
				return next(err);
			}
...
uploadsController.uploadTouchIcon = (req, res, next) => {
	var uploadedFile = req.files.files[0];
	var allowedTypes = ['image/png'];
	var sizes = [36, 48, 72, 96, 144, 192];

	if (validateUpload(req, res, next, uploadedFile, allowedTypes)) {
		file.saveFileToLocal('touchicon-orig.png', 'system', uploadedFile.path, (err, imageObj) => {
			if (err) {
				return next(err);
			}

			// Resize the image into squares for use as touch icons at various DPIs
			async.each(sizes, (size, next) => {
				async.series([
...
uploadsController.uploadLogo = (req, res, next) => {
	upload('site-logo', req, res, next);
};

uploadsController.uploadSound = (req, res, next) => {
	var uploadedFile = req.files.files[0];
...
uploadsController.uploadDefaultAvatar = (req, res, next) => {
	upload('avatar-default', req, res, next);
};

uploadsController.uploadOgImage = (req, res, next) => {
