...
sitemap.getPages = (callback) => {
	...
	plugins.fireHook('filter:sitemap.getPages', { urls: urls }, (err, data) => {
		if (err) {
			return callback(err);
		}
...
sitemap.getCategories = (callback) => {
	...
	var categoryUrls = [];
	categories.getCategoriesByPrivilege('categories:cid', 0, 'find', (err, categoriesData) => {
		if (err) {
			return callback(err);
		}
...
sitemap.getTopicPage = (page, callback) => {
