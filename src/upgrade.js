...
Upgrade.getAll = (callback) => {
	async.waterfall([
		...
		(files, next) => {
			...
			var versionA;
			var versionB;
			setImmediate(next, null, files.filter(file => path.basename(file) !== 'TEMPLATE').sort((a, b) => {
...
Upgrade.appendPluginScripts = (files, callback) => {
	async.waterfall([
...
Upgrade.check = (callback) => {
	...
	async.waterfall([
		...
		(files, next) => {
			db.getSortedSetRange('schemaLog', 0, -1, (err, executed) => {
				if (err) {
					return callback(err);
				}

				var remainder = files.filter(name => executed.indexOf(path.basename(name, '.js')) === -1);
...
Upgrade.run = (callback) => {
	...
	async.parallel({
		...
		queue = data.available.reduce((memo, cur) => {
...
Upgrade.runParticular = (names, callback) => {
	console.log('\nParsing upgrade scripts... ');

	async.waterfall([
		async.apply(file.walk, path.join(__dirname, './upgrades')),
		(files, next) => {
			var upgrades = files.filter(file => names.indexOf(path.basename(file, '.js')) !== -1);
...
Upgrade.process = (files, skipCount, callback) => {
...
Upgrade.incrementProgress = (value) => {
