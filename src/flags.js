...
Flags.init = (callback) => {
	...
	var prepareSets = (sets, orSets, prefix, value) => {
...
Flags.list = (filters, uid, callback) => {
	...
	async.waterfall([
		...
		(flagIds, next) => {
			...
			if (orSets.length) {
				db.getSortedSetRevUnion({ sets: orSets, start: 0, stop: -1, aggregate: 'MAX' }, (err, _flagIds) => {
					if (err) {
						return next(err);
					}
		...
		(flagIds, next) => {
			async.map(flagIds, (flagId, next) => {
				async.waterfall([
					async.apply(db.getObject, 'flag:' + flagId),
					(flagObj, next) => {
						user.getUserFields(flagObj.uid, ['username', 'picture'], (err, userObj) => {
...
Flags.validate = (payload, callback) => {
	async.parallel({
		...
		switch (payload.type) {
		case 'post':
			privileges.posts.canEdit(payload.id, payload.uid, (err, editable) => {
				if (err) {
					return callback(err);
				}
		...
		case 'user':
			privileges.users.canEdit(payload.uid, payload.id, (err, editable) => {
				if (err) {
					return callback(err);
				}
...
Flags.getNotes = (flagId, callback) => {
	async.waterfall([
		...
		(notes, uids, next) => {
			user.getUsersFields(uids, ['username', 'userslug', 'picture'], (err, users) => {
				if (err) {
					return next(err);
				}

				next(null, notes.map((note, idx) => {
					note.user = users[idx];
					return note;
				}));
			});
...
Flags.create = (type, id, uid, reason, timestamp, callback) => {
	var targetUid;
	var targetCid;
...
Flags.exists = (type, id, uid, callback) => {
	db.isSortedSetMember('flags:hash', [type, id, uid].join(':'), callback);
};

Flags.getTarget = (type, id, uid, callback) => {
	async.waterfall([
		async.apply(Flags.targetExists, type, id),
		(exists, next) => {
			if (exists) {
				...
				case 'user':
					user.getUsersData([id], (err, users) => {
...
Flags.targetExists = (type, id, callback) => {
	switch (type) {
	case 'post':
...
Flags.getTargetUid = (type, id, callback) => {
	switch (type) {
	case 'post':
...
Flags.getTargetCid = (type, id, callback) => {
	switch (type) {
	case 'post':
...
Flags.update = (flagId, uid, changeset, callback) => {
	...
	var notifyAssignee = (assigneeId, next) => {
		if (assigneeId === '') {
			// Do nothing
			return next();
		}
	...
	async.waterfall([
		...
		(current, next) => {
			...
			async.parallel(tasks, err => next(err));
		},
	], callback);
};

Flags.getHistory = (flagId, callback) => {
	var history;
	var uids = [];
	async.waterfall([
		async.apply(db.getSortedSetRevRangeWithScores.bind(db), 'flag:' + flagId + ':history', 0, -1),
		(_history, next) => {
			history = _history.map((entry) => {
				try {
...
Flags.appendHistory = (flagId, uid, changeset, callback) => {
	var payload;
...
Flags.appendNote = (flagId, uid, note, datetime, callback) => {
...
Flags.notify = (flagObj, uid, callback) => {
