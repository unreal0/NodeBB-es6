...
module.exports = (Groups) => {
	Groups.update = (groupName, values, callback) => {
	...
	Groups.hide = (groupName, callback) => {
		showHide(groupName, 'hidden', callback);
	};

	Groups.show = (groupName, callback) => {
	...
	Groups.renameGroup = (oldName, newName, callback) => {
		...
		async.waterfall([
			...
			(exists, next) => {
				...
				async.series([
					...
					(next) => {
						db.getSortedSetRange('groups:createtime', 0, -1, (err, groups) => {
							if (err) {
								return next(err);
							}
