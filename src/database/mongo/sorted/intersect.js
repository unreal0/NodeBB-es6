module.exports = (db, module) => {
	module.sortedSetIntersectCard = (keys, callback) => {
		...
		db.collection('objects').aggregate(pipeline, (err, data) => {
			callback(err, Array.isArray(data) && data.length ? data[0].count : 0);
		});
	};


	module.getSortedSetIntersect = (params, callback) => {
		params.sort = 1;
		getSortedSetRevIntersect(params, callback);
	};

	module.getSortedSetRevIntersect = (params, callback) => {
		params.sort = -1;
		getSortedSetRevIntersect(params, callback);
	};
