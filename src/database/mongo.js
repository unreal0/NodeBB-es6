...
mongoModule.init = (callback) => {
	...
	mongoClient.connect(connString, connOptions, (err, _db) => {
		if (err) {
		...
		require('./mongo/sets').default(db, mongoModule);
...
mongoModule.initSessionStore = (callback) => {
...
mongoModule.createIndices = (callback) => {
...
mongoModule.info = (db, callback) => {
	...
	async.waterfall([
		...
		(results, next) => {
			...
			results.listCollections = results.listCollections.map(collectionInfo => (
				{
...
function getCollectionStats(db, callback) {
	async.waterfall([
		...
		(items, next) => {
			async.map(items, (collection, next) => {
