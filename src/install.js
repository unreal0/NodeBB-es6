...
function setupDefaultConfigs(next) {
	...
	meta.configs.setOnEmpty(defaults, (err) => {
		if (err) {
			return next(err);
		}
...
function createAdministrator(next) {
	...
	Groups.getMemberCount('administrators', (err, memberCount) => {
		if (err) {
			return next(err);
		}
		if (memberCount > 0) {
...
function createAdmin(callback) {
	...
	function retryPassword(originalResults) {
		...
		prompt.get(passwordQuestions, (err, results) => {
			if (!results) {
				return callback(new Error('aborted'));
			}
...
function createCategories(next) {
	...
	Categories.getAllCategories(0, (err, categoryData) => {
		if (err) {
			return next(err);
		}
		...
		fs.readFile(path.join(__dirname, '../', 'install/data/categories.json'), 'utf8', (err, default_categories) => {
			if (err) {
				return next(err);
			}
...
function createMenuItems(next) {
	...
	db.exists('navigation:enabled', (err, exists) => {
		if (err || exists) {
			return next(err);
		}
...
function enableDefaultPlugins(next) {
	...
	defaultEnabled = defaultEnabled.filter((plugin, index, array) => array.indexOf(plugin) === index);
...
install.save = (server_conf, callback) => {
	...
	fs.writeFile(serverConfigPath, JSON.stringify(server_conf, null, 4), (err) => {
		if (err) {
