...
function onMessage(socket, payload) {
	...
	var methodToCall = parts.reduce((prev, cur) => {
...
function checkMaintenance(socket, callback) {
	...
	user.isAdministrator(socket.uid, (err, isAdmin) => {
		if (err || isAdmin) {
			return callback(err);
		}
	});
...
function validateSession(socket, callback) {
	...
	db.sessionStore.get(req.signedCookies[nconf.get('sessionKey')], (err, sessionData) => {
		if (err || !sessionData) {
...
function authorize(socket, callback) {
	...
	async.waterfall([
		...
		(next) => {
			db.sessionStore.get(request.signedCookies[nconf.get('sessionKey')], (err, sessionData) => {
				if (err) {
					return next(err);
				}
...
Sockets.in = room => io.in(room);

Sockets.getUserSocketCount = (uid) => {
	if (!io) {
		return 0;
	}
...
Sockets.reqFromSocket = (socket, payload, event) => {
