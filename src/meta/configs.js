...
Configs.get = (field, callback) => {
	db.getObjectField('config', field, callback);
};

Configs.getFields = (fields, callback) => {
	db.getObjectFields('config', fields, callback);
};

Configs.set = (field, value, callback) => {
...
function processConfig(data, callback) {
	async.parallel([
		...
		(next) => {
			...
			if (data['brand:logo']) {
				image.size(path.join(nconf.get('upload_path'), 'system', 'site-logo-x50.png'), (err, size) => {
					if (err) {
						return next(err);
					}
...
Configs.setOnEmpty = (values, callback) => {
	async.waterfall([
		(next) => {
