...
var targetHandlers = {
	'plugin static dirs': (parallel, callback) => {
		meta.js.linkStatics(callback);
	},
	'requirejs modules': (parallel, callback) => {
		meta.js.buildModules(parallel, callback);
	},
	'client js bundle': (parallel, callback) => {
		meta.js.buildBundle('client', parallel, callback);
	},
	'admin js bundle': (parallel, callback) => {
	...
	'client side styles': (parallel, callback) => {
		meta.css.buildBundle('client', parallel, callback);
	},
	'admin control panel styles': (parallel, callback) => {
	...
	templates: (parallel, callback) => {
		meta.templates.compile(callback);
	},
	languages: (parallel, callback) => {
		meta.languages.build(callback);
	},
	sounds: (parallel, callback) => {
...
aliases = Object.keys(aliases).reduce((prev, key) => {
	var arr = aliases[key];
	arr.forEach((alias) => {
		prev[alias] = key;
	});
...
var allTargets = Object.keys(targetHandlers).filter(name => typeof targetHandlers[name] === 'function');
function buildTargets(targets, parallel, callback) {
	var all = parallel ? async.each : async.eachSeries;

	var length = Math.max.apply(Math, targets.map(name => name.length));

	all(targets, (target, next) => {
