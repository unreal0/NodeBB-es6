...
resetCommand
	...
	.action((options) => {
		var valid = ['theme', 'plugin', 'widgets', 'settings', 'all'].some(x => options[x]);
		if (!valid) {
		...
		require('./reset').reset(options, (err) => {
			if (err) { throw err; }
			require('../meta/build').buildAll((err) => {
				if (err) { throw err; }
...
program
	...
	.action((name) => {
		...
		var command = program.commands.find(command => command._name === name);
		if (command) {
			command.help();
		} else {
			program.help();
