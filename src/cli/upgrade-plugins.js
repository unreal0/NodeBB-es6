...
function getModuleVersions(modules, callback) {
	...
	async.eachLimit(modules, 50, (module, next) => {
		fs.readFile(path.join(dirname, 'node_modules', module, 'package.json'), { encoding: 'utf-8' }, (err, pkg) => {
			if (err) {
				return next(err);
			}
...
function getInstalledPlugins(callback) {
	async.parallel({
		...
		payload.bundled = payload.bundled.filter(pkgName => isNbbModule.test(pkgName));
		payload.deps = payload.deps.filter(pkgName => isNbbModule.test(pkgName));

		// Whittle down deps to send back only extraneously installed plugins/themes/etc
		checklist = payload.deps.filter((pkgName) => {
...
function getCurrentVersion(callback) {
	fs.readFile(path.join(dirname, 'install/package.json'), { encoding: 'utf-8' }, (err, pkg) => {
		if (err) {
			return callback(err);
		}
	...
	async.waterfall([
		...
		(payload, next) => {
			...
			request({
				...
				var current;
				var suggested;
				var upgradable = body.map((suggestObj) => {
	...
	checkPlugins(standalone, (err, found) => {
		...
		prompt.get({
			...
			if (['y', 'Y', 'yes', 'YES'].indexOf(result.upgrade) !== -1) {
				console.log('\nUpgrading packages...');
				var args = packageManagerInstallArgs.concat(found.map(suggestObj => suggestObj.name + '@' + suggestObj.suggested));
