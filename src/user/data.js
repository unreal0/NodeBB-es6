...
module.exports = (User) => {
	...
	User.getUserField = (uid, field, callback) => {
		User.getUserFields(uid, [field], (err, user) => {
			callback(err, user ? user[field] : null);
		});
	};

	User.getUserFields = (uid, fields, callback) => {
		User.getUsersFields([uid], fields, (err, users) => {
	...
	User.getUsersFields = (uids, fields, callback) => {
		...
		var uniqueUids = uids.filter((uid, index) => index === uids.indexOf(uid));
	...
	User.getMultipleUserFields = (uids, fields, callback) => {
	...
	User.getUserData = (uid, callback) => {
		User.getUsersData([uid], (err, users) => {
			callback(err, users ? users[0] : null);
		});
	};
	...
	function uidsToUsers(uids, uniqueUids, usersData) {
		var ref = uniqueUids.reduce((memo, cur, idx) => {
			memo[cur] = idx;
			return memo;
		}, {});
		var users = uids.map(uid => usersData[ref[uid]]);
		return users;
	}
	...
	User.incrementUserFieldBy = (uid, field, value, callback) => {
		incrDecrUserFieldBy(uid, field, value, 'increment', callback);
	};

	User.decrementUserFieldBy = (uid, field, value, callback) => {
