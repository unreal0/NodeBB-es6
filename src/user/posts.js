...
module.exports = (User) => {
	User.isReadyToPost = (uid, cid, callback) => {
		isReady(uid, cid, 'lastposttime', callback);
	};

	User.isReadyToQueue = (uid, cid, callback) => {
	...
	User.onNewPostMade = (postData, callback) => {
		async.series([
			(next) => {
	...
	User.addPostIdToUser = (uid, pid, timestamp, callback) => {
		db.sortedSetAdd('uid:' + uid + ':posts', timestamp, pid, callback);
	};

	User.incrementUserPostCountBy = (uid, value, callback) => {
	...
	User.getPostIds = (uid, start, stop, callback) => {
		db.getSortedSetRevRange('uid:' + uid + ':posts', start, stop, (err, pids) => {
