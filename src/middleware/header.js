...
module.exports = (middleware) => {
	...
	middleware.renderHeader = (req, res, data, callback) => {
		...
		async.waterfall([
			(next) => {
				async.parallel({
					...
					isEmailConfirmSent: (next) => {
						if (!meta.config.requireEmailConfirmation || !req.uid) {
							return next(null, false);
						}
						db.get('uid:' + req.uid + ':confirm:email:sent', next);
					},
					languageDirection: (next) => {
						translator.translate('[[language:dir]]', res.locals.config.userLang, (translated) => {
							next(null, translated);
						});
					},
					browserTitle: (next) => {
						translator.translate(controllers.helpers.buildTitle(translator.unescape(data.title)), (translated) => {
							next(null, translated);
						});
					},
			...
			(results, next) => {
				...
				results.navigation = results.navigation.map((item) => {
	...
	middleware.renderFooter = (req, res, data, callback) => {
		async.waterfall([
			(next) => {
			...
			(data, results, next) => {
				data.templateValues.scripts = results.scripts.map(script => ({ src: script }));
